...
function removeFirstOccurrences(str, value) {
  const vInd = str.indexOf(value);
  return vInd !== -1
    ? str.slice(0, vInd) + str.slice(vInd + value.length)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const vInd = str.lastIndexOf(value);
  return vInd !== -1
    ? str.slice(0, vInd) + str.slice(vInd + value.length)
    : str;
}
...
function reverseString(str) {
  return !str ? str : reverseString(str.substring(1)).concat(str.charAt(0));
}
...
function countVowels(str) {
  const stStr = 'ayeiouAYEIOU';
  let result = 0;
  for (let i = 0; i < str.length; i += 1) {
    result += stStr.includes(str.charAt(i)) ? 1 : 0;
  }
  return result;
}
...
function isPalindrome(str) {
  const compressedStr = str.toLowerCase().replace(/\W/g, '');
  const reversedStr = compressedStr.split('').reverse().join('');
  return Boolean(compressedStr.localeCompare(reversedStr)) !== true;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentArray.length; i += 1) {
    bigWord = sentArray[i].length > bigWord.length ? sentArray[i] : bigWord;
  }
  return bigWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < sentArray.length; i += 1) {
    sentArray[i] = sentArray[i].split('').reverse().join('');
  }
...
function encodeToRot13(str) {
  const simpStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let encrStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const symbInd = simpStr.indexOf(str.charAt(i));
    const newSymbInd = rot13.charAt(symbInd);
    encrStr += str[i].match(/\W/g) ? str[i] : newSymbInd;
  }
  return encrStr;
}
